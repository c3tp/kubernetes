...
func generateCredentials(svcNamespace, name, svcName, localDNSZoneName, serverCredName string, ips, hostnames []string, enableHTTPBasicAuth, enableTokenAuth, enableWebhookAuth, dryRun bool) (*credentials, error) {
	...
	if enableWebhookAuth {
		credentials.webhookUrl = string(uuid.newUUID())
...
func createAPIServerCredentialsSecret(clientset client.Interface, namespace, credentialsName, federationName string, credentials *credentials, dryRun bool) (*api.Secret, error) {
	...
	if credentials.webhookUrl != "" {
		...
		data["ca.pem"] = certificateData(credentials.webhookca)
		data["cert.pem"] = certificateData(credentials.webhookcert)
		data["key.pem"] = certificateData(credentials.clientkey)
	}
	secret := &api.Secret{
		ObjectMeta: metav1.ObjectMeta{
